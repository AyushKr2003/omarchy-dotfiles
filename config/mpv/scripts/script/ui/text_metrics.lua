...
local function glyph_advance(character, value)
  if value == 0x09 then return 1900 * 4 end
  if value == 0x0A or value == 0x0D or is_combining(value) then return 0 end
  if is_wide(value) then return 2485 end
  -- Since we use JetBrainsMono Nerd Font (monospaced), every ASCII and Latin-1
  -- character has an advance of approximately 55% of FONT_HEIGHT (3451 * 0.55 = 1900).
  if (value >= 0x20 and value <= 0x7E) or (value >= 0xA0 and value <= 0xFF) then
    return 1900
  end
