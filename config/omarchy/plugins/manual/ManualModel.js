...
// title from the "NN-some-slug.md" or "docs/some-guide.md" filename.
function titleFromFilename(name) {
  var base = String(name || "").replace(/\.md$/i, "").replace(/^\d+-/, "").replace(/^docs\//i, "")
  var words = base.split(/[-_]/).filter(function(w) { return w.length > 0 })
...
// number if numbered, with README.md first for plugins, then alphabetical.
function parseChapterIndex(raw) {
  ...
  chapters.sort(function(a, b) {
    ...
    var aIsReadme = a.file.toLowerCase() === "readme.md"
    var bIsReadme = b.file.toLowerCase() === "readme.md"
...
function parseSearchResults(raw, chapters, limit) {
  ...
  for (var i = 0; i < lines.length && results.length < cap; i++) {
    var match = lines[i].match(/^(.*?([^/:]+(?:\/[^/:]+)?\.md)):(\d+):(.*)$/i)
    if (!match) continue
    var fullPath = match[1]
    var chapterIndex = -1
    for (var c = 0; c < chapters.length; c++) {
      if (chapters[c].file === match[2] || fullPath.endsWith("/" + chapters[c].file) || fullPath === chapters[c].file) {
        chapterIndex = c
        break
      }
    }
    ...
    results.push({
      ...
      file: chapters[chapterIndex].file,
...
function resolveChapter(chapters, ref) {
  ...
  if (!wanted || !Array.isArray(chapters)) return -1
  ...
  if (wanted.match(/^\d+$/)) {
    ...
    for (i = 0; i < chapters.length; i++)
      if (chapters[i].number === number && chapters[i].hasNumber) return i
  }
  for (i = 0; i < chapters.length; i++) {
    var fileLower = String(chapters[i].file || "").toLowerCase()
    var slugLower = String(chapters[i].slug || "").toLowerCase()
    if (chapters[i].file === wanted || fileLower === lower || slugLower === lower) return i
  }
  for (i = 0; i < chapters.length; i++) {
    var slugLower = String(chapters[i].slug || "").toLowerCase()
    if (slugLower.indexOf(lower) !== -1) return i
  }
  for (i = 0; i < chapters.length; i++) {
    var titleLower = String(chapters[i].title || "").toLowerCase()
    if (titleLower.indexOf(lower) !== -1) return i
  }
  return -1
}

// Resolve a user-supplied source reference (plugin id, directory name, slug, or title)
// to a source index in the sources array.
function resolveSource(sources, ref) {
  var wanted = String(ref || "").trim()
  if (!wanted || !Array.isArray(sources)) return -1
  var lower = wanted.toLowerCase()
  var i

  // Exact ID match
  for (i = 0; i < sources.length; i++) {
    if (sources[i].id === wanted || sources[i].id.toLowerCase() === lower) return i
  }
  // Directory name match
  for (i = 0; i < sources.length; i++) {
    if (sources[i].dirName && sources[i].dirName.toLowerCase() === lower) return i
  }
  // Name / title exact match
  for (i = 0; i < sources.length; i++) {
    if (sources[i].name && sources[i].name.toLowerCase() === lower) return i
  }
  // Substring match in id or name
  for (i = 0; i < sources.length; i++) {
    if (sources[i].id.toLowerCase().indexOf(lower) !== -1 || (sources[i].name && sources[i].name.toLowerCase().indexOf(lower) !== -1)) return i
  }
  return -1
}
...
function linkTarget(href) {
  ...
  if (file.match(/\.md$/i)) return { kind: "chapter", file: file }
  return { kind: "none" }
}
